...
import { 
  ...
  RotateCcw, RotateCw, Power, Activity, 
      ...
      <main className="max-w-7xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="relative aspect-video bg-slate-950 rounded-[40px] border border-white/10 shadow-2xl overflow-hidden group">
            ...
            <div className="absolute top-6 left-6 flex gap-2">
              <span className="px-3 py-1 bg-black/60 backdrop-blur-md border border-white/10 rounded-full text-[10px] font-bold">LIVE FEED</span>
            </div>
          </div>
          ...
          <div className="bg-[#111a2e]/50 backdrop-blur-md rounded-[30px] border border-white/5 p-6 shadow-inner">
            ...
            <div className="space-y-1 h-32 overflow-y-auto font-mono text-[13px]">
              {logs.map((log, i) => (
                <div key={i} className={log.includes('IZVRŠENO') ? 'text-cyan-400' : 'text-slate-500'}>{log}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-[#111a2e]/50 backdrop-blur-xl rounded-[40px] p-8 border border-white/10 shadow-2xl text-center">
            <h3 className="text-white font-black text-sm uppercase tracking-widest mb-8">Upravljanje</h3>
            ...
            <div className="flex flex-col items-center gap-6">
              ...
              <div className="flex gap-4 mt-2">
                <div className="flex flex-col items-center gap-2">
                  <span className="text-[9px] font-bold text-slate-500">ROT L (Q)</span>
                  <ControlButton icon={<RotateCcw />} active={activeKey === 'Q'} onDown={() => sendCommand('rotiraj_levo')} onUp={() => sendCommand('stop')} variant="cyan" />
                </div>
                <div className="flex flex-col items-center gap-2">
                  <span className="text-[9px] font-bold text-slate-500">ROT R (E)</span>
...
function ControlButton({ icon, active, onDown, onUp, variant = "default" }: any) {
  const styles = active 
    ? (variant === "cyan" ? "bg-cyan-500 text-black border-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.4)]" : "bg-white text-black border-white shadow-xl")
